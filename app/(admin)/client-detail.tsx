...
export default function ClientDetail() {
  ...
  useEffect(() => {
    (async () => {
      ...
      setC(client); setProjects(projs ?? []); setLoading(false);
    })();
  }, [id]);
  ...
  if (!c) return <View style={s.center}><Text style={{ color: colors.muted }}>Client not found.</Text></View>;
...
function Row({ k, v, last }: { k: string; v: string; last?: boolean }) {
  return (
    <View style={[rs.row, last && { borderBottomWidth: 0 }]}>
      <Text style={rs.k}>{k}</Text><Text style={rs.v}>{v}</Text>
    </View>
  );
}
const rs = StyleSheet.create({
  ...
  k: { fontFamily: "Inter", fontSize: 13.5, color: colors.muted }, v: { fontFamily: "InterBold", fontSize: 13.5, color: colors.ink, flexShrink: 1, textAlign: "right" },
});
const s = StyleSheet.create({
  ...
  name: { fontFamily: "JakartaBold", fontSize: 20, color: colors.ink }, company: { fontFamily: "Inter", fontSize: 13, color: colors.muted, marginTop: 2 },
  ...
  projName: { fontFamily: "InterBold", fontSize: 14, color: colors.ink }, projStatus: { fontFamily: "Inter", fontSize: 12.5, color: colors.muted },
});