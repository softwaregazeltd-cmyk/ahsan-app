...
export default function ProjectDetail() {
  ...
  const load = useCallback(async () => {
    ...
    setP(proj ?? null); setDelivs(dl ?? []); setLoading(false);
  }, [id]);
  ...
  function onPickStatus(status: string) {
    if (status === "On Hold" || status === "Cancelled") {
      setSheetStatus(status); // opens reason sheet
    } else {
      applyStatus(status);
    }
  }
  ...
  async function postDeliverable() {
    ...
    const { error } = await supabase.from("deliverables").insert({ project_id: id, name: dName.trim(), link: dLink.trim() || null });
    if (error) { Alert.alert("Error", error.message); return; }
    setDName(""); setDLink(""); load();
  }
        ...
        {delivs.length === 0 ? <Text style={s.empty}>No deliverables yet.</Text> :
          delivs.map((d) => (
            <View key={d.id} style={s.delivRow}>
              ...
              <Pill
                label={d.status}
                tone={d.status === "Approved" ? "ok" : d.status === "Changes" ? "red" : "amber"}
              />
            </View>
          ))
        }
...
const s = StyleSheet.create({
  ...
  k: { fontFamily: "Inter", fontSize: 12.5, color: colors.muted }, v: { fontFamily: "InterBold", fontSize: 12.5, color: colors.ink },
  ...
  dotActive: { borderColor: colors.primary, backgroundColor: colors.primary }, dotDone: { borderColor: colors.ok, backgroundColor: colors.ok },
  mileName: { flex: 1, fontFamily: "InterBold", fontSize: 13.5, color: colors.ink }, mileState: { fontFamily: "Inter", fontSize: 11, color: colors.muted },
  ...
  delivName: { fontFamily: "InterBold", fontSize: 13.5, color: colors.ink }, delivLink: { fontFamily: "Inter", fontSize: 11, color: colors.muted, marginTop: 2 },
